<!-- footer -->
<footer class="light-footer">
    <div class="widget-area">
        <div class="container">
            <div class="row">

                <div class="col-md-4 widget">
                    <div class="about-widget">
                        <div class="widget-title-image">
                            <img style="width:250px;max-height:100px!important" src="{{asset('./images/logo-lg.png')}}" alt="">
                        </div>
                        <p>
                          Το Γεωπονικό Kέντρο Ιστιαίας είναι κατάστημα λιανικής πώλησης γεωργικών φαρμάκων, λιπασμάτων και λοιπών αγροτικών εφοδίων.
                        </p>
                    </div><!-- / about-widget -->
                </div><!-- / widget -->

                <!-- / third widget -->

                <div class="col-md-4 pull-right widget">
                    <div class="widget-title">
                        <h4>ΕΠIΚΟΙΝΩΝΙΑ</h4>
                    </div>
                    <div class="contact-widget">
                        <div class="info">
                            <p><i class="lnr lnr-map-marker"></i><span>1o χλμ Ιστιαίας-Αιδηψού</span></p>
                        </div>
                        <div class="info">
                            <a href="tel:+0123456789"><i class="lnr lnr-phone-handset"></i><span>+30 22260 53649</span></a>
                        </div>
                        <div class="info">
                            <a href="mailto:baroutis.d.86@gmail.com"><i class="lnr lnr-envelope"></i><span>baroutis.d.86@gmail.com</span></a>
                        </div>
                        <div class="info">
                            <i class="lnr lnr-thumbs-up"></i>
                            <span class="social text-left">
                                <a class="no-margin" href="#"><i class="fa fa-facebook"></i></a>
                                <a href="#"><i class="fa fa-twitter"></i></a>
                                <a href="#"><i class="fa fa-google-plus"></i></a>
                                <a href="#"><i class="fa fa-linkedin"></i></a>
                                <a href="#"><i class="fa fa-pinterest"></i></a>
                            </span>
                        </div>
                    </div><!-- / contact-widget -->
                </div><!-- / widget -->
                <!-- / fourth widget -->

            </div><!-- / row -->
        </div><!-- / container -->
    </div><!-- / widget-area -->
    <div class="footer-info">
        <div class="container">
                <div class="pull-left copyright">
                    <p><small><strong>(c) 2017-2018 tsiktsiris@sch.gr</small></strong></p>
                </div>
                <span class="pull-right">
                    <img src="{{asset('images/visa.png')}}" alt="">
                    <img src="{{asset('images/mastercard.png')}}" alt="">
                    <img src="{{asset('images/discover.png')}}" alt="">
                    <img src="{{asset('images/paypal.png')}}" alt="">
                </span>
        </div><!-- / container -->
    </div><!-- / footer-info -->
</footer>
<!-- / footer -->
